...
- **`taiko-tdx-prover`** — current taiko / raiko2 / shasta build.
...
- `l1_genesis_time`: This is the genesis time of the L1 chain in seconds since the Unix epoch. It is used in raiko2
...
2. **Initialize submodules** (required for raiko2 source, first time only):
   ```bash
   make submodules
   ```
...
3. **Build a specific image**:
   ```bash
   # Build the taiko (raiko2 + shasta) TDX prover image
