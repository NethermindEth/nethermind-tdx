...
- The **attestation issuer is no longer set in `env.json`** — it is derived from the
  build profile (see [Building Images](#building-images)): `make build AZURE=true`
  produces the `azure` issuer (Azure vTPM-bound TDX quote), while `GCP=true` and
  bare-metal builds produce the native `tdx` issuer (DCAP quote read straight from
  `/dev/tdx_guest`). This keeps the issuer in lockstep with the target platform.
